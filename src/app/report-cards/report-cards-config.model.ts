...
export class AzureSearchRequest {
  ...
  recordsPerPage = 1;
  get totalMatchingRecords() {
    return this._totalMatchingRecords;
  }
  ...
  get totalPagesAvailable() {
    return this._totalPagesAvailable;
  }
  ...
  private setTotalPagesAvailable() {
    const calculatedPagesAvailable = Math.ceil(
      this._totalMatchingRecords / this.recordsPerPage
    );

    return calculatedPagesAvailable <= MAX_AZURE_SEARCH_PAGES
      ? calculatedPagesAvailable
